...
struct weatherstation_t
{
  ...
  weatherstation_t(std::string ID,
                   std::string Date,
                   std::string Time,
                   std::string Name,
                   std::string Lat,
...
public:
  ...
  {
    short answer = 0;
    const auto Date = restinio::utils::unescape_percent_encoding(params["Date"]);
  ...
  auto on_post_data(const restinio::request_handle_t &req, rr::route_params_t)
  {
    ...
    try
    {
      m_weather.emplace_back(json_dto::from_json<weatherstation_t>(req->body()));
                         ...
                         rr::route_params_t params)
  {
    ...
    try
    {
      ...
      {
        mark_as_bad_request(resp);
        resp.set_body("No weather with #" + ID + "\n");
      }
    }
    catch (const std::exception & /*ex*/)
    {
                         ...
                         rr::route_params_t params)
  {
    ...
    if (0 != ID && ID <= m_weather.size())
    {
      const auto &b = m_weather[ID - 1];
      resp.set_body("Delete weather #" + ID + ": " +
                    b.m_Date + "[" + b.m_ID + "]\n");

      m_weather.erase(m_weather.begin() + (ID - 1));
    }
    else
    {
      resp.set_body("No weather with #" + ID + "\n");

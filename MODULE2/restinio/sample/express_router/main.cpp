...
struct weather_t
{
	...
	weather_t(
		...
		std::string Humidity)
		: m_ID{std::move(ID)}, m_Date{std::move(Date)}, m_Time{std::move(Time)}, m_Name{std::move(Name)}, m_Lat{std::move(Lat)}, m_Lon{std::move(Lon)}, m_Temperature{std::move(Temperature)}, m_Humidity{std::move(Humidity)}
	{
	}
	...
	json_io(JSON_IO &io)
	{
		io
			...
			json_dto::mandatory("Date", m_Date) & json_dto::mandatory("Time", m_Time) & json_dto::mandatory("Name", m_Name) & json_dto::mandatory("Lat", m_Lat) & json_dto::mandatory("Lon", m_Lon) & json_dto::mandatory("Temperature", m_Temperature) & json_dto::mandatory("Humidity", m_Humidity);
	}
...
public:
		...
		const restinio::request_handle_t &req, rr::route_params_t params)
	{
		...
		try
		{
			...
			sendMessage("PUT: id = " + json_dto::from_json<weather_t>(req->body()).m_id);
		}
		catch (const std::exception & /*ex*/)
		{
			mark_as_bad_request(resp);
		...
		const restinio::request_handle_t &req, rr::route_params_t params)
	{
		...
		if (0 != weathernum && weathernum <= m_weathers.size())
		{
			...
			sendMessage("DELETE: id = " + json_dto::from_json<weather_t>(req->body()).m_id);
		}
		else
		{
			resp.set_body(
